...
def _ensure_guest_session(db: Session, session_token: Optional[str], metadata: Optional[Dict[str, Any]] = None) -> Optional[GuestSession]:
    token = (session_token or "").strip()
    if not token:
        return None
    guest = db.query(GuestSession).filter(GuestSession.session_token == token).first()
    if not guest:
        guest = GuestSession(session_token=token, metadata_=(metadata or {}))
        db.add(guest)
        db.flush()
    else:
        current = dict(getattr(guest, "metadata_", {}) or {})
        if metadata:
            current.update(metadata or {})
        guest.metadata_ = current
...
def _resolve_session_identity(
    *,
    ...
    guest_session = _ensure_guest_session(db, session_token, metadata)
    ...
    session = IntakeSession(
        ...
        session_token=session_token or (guest_session.session_token if guest_session else None),
