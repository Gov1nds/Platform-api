...
# -------------------------------------------------------------------
# 🔴 UPDATED: STRONG ROW HASH (NO LOGIC LOSS)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 🔴 MAIN: CREATE BOM (UPGRADED, ZERO LOGIC LOSS)
# -------------------------------------------------------------------
...
def create_bom_from_analyzer(
    ...
    raw_count = sum(
        1 for c in components if (c.get("category") or "").lower() == "raw_material"
    )
    ...
    for idx, comp in enumerate(components):
        ...
        # -------------------------------------------------------------------
        # 🔴 UPDATED BOMPart INSERT (NO LOGIC LOSS + ENHANCED METADATA)
        # -------------------------------------------------------------------
        ...
        db.add(BOMPart(
            ...
            metadata_={
                ...
                # 🔴 NEW: failure + parse metadata
                "parse_status": comp.get("parse_status"),
                "parse_reason_code": comp.get("parse_reason_code"),
                "parse_reason": comp.get("parse_reason"),
                "failure_metadata": comp.get("failure_metadata"),
...
def get_bom_parts_as_dicts(db: Session, bom_id: str) -> List[Dict[str, Any]]:
    ...
    for p in parts:
        ...
        result.append({
            ...
            "source_row": meta.get("source_row", p.source_row),

            "raw_text": p.raw_text,
            "standard_text": p.normalized_text,
