...
def _get_project_for_rfq(db: Session, rfq: Optional[RFQ]):
    if not rfq or not getattr(rfq, "bom_id", None):
        return None
...
def _update_project_from_fulfillment(db: Session, rfq: RFQ, state: str, pointer_payload: Dict[str, Any], actor: Optional[str] = None):
    project = _get_project_for_rfq(db, rfq)
    ...
    project.current_rfq_id = pointer_payload.get("rfq_id", getattr(project, "current_rfq_id", None) or (rfq.id if rfq else None))
    project.current_rfq_batch_id = pointer_payload.get("rfq_batch_id", getattr(project, "current_rfq_batch_id", None) or (rfq.id if rfq else None))
    ...
    vendor_id = pointer_payload.get("vendor_id") or getattr(rfq, "selected_vendor_id", None)
    if vendor_id:
    ...
    project.project_metadata["current_rfq_id"] = project.current_rfq_id
    project.project_metadata["current_rfq_batch_id"] = project.current_rfq_batch_id
    if vendor_id:
        project.project_metadata["current_vendor_id"] = vendor_id
        project.project_metadata["selected_vendor_id"] = vendor_id
    _record_event_safe(
    ...
    if current_idx >= len(stages) - 1:
        ...
        if rfq:
            ...
            _update_project_from_fulfillment(
                ...
                rfq,
                FulfillmentState.closed.value,
                {"tracking_stage": TrackingStage.T4.value, "vendor_id": rfq.selected_vendor_id},
                actor=updated_by,
            )
        return tracking
...
def create_shipment(
    ...
    if rfq:
        _update_project_from_fulfillment(
            ...
            {"po_id": po.id, "shipment_id": shipment.id, "tracking_stage": TrackingStage.T4.value, "vendor_id": po.vendor_id},
            actor=user.id if user else None,
        )
...
def confirm_goods_receipt(
    ...
    if rfq:
        _update_project_from_fulfillment(
            ...
            rfq,
            FulfillmentState.receipt_confirmed.value,
            {"po_id": po.id, "shipment_id": shipment_id, "tracking_stage": TrackingStage.T4.value, "vendor_id": po.vendor_id},
            actor=user.id if user else None,
        )
...
def create_invoice(
    ...
    if rfq:
        _update_project_from_fulfillment(
            ...
            rfq,
            FulfillmentState.invoice_matched.value,
            {"po_id": po.id, "invoice_id": inv.id, "tracking_stage": TrackingStage.T4.value, "vendor_id": inv.vendor_id},
            actor=user.id if user else None,
        )
        ...
        if po:
            ...
            if rfq:
                _update_project_from_fulfillment(
                    ...
                    rfq,
                    FulfillmentState.closed.value,
                    {"po_id": po.id, "invoice_id": inv.id, "tracking_stage": TrackingStage.T4.value, "vendor_id": po.vendor_id},
                    actor=user.id if user else None,
                )
    db.flush()
    return payment
